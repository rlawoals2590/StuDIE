...
def local_ranking(local, point, user_id):
    ...
    local_rank = [[i[2], i[0], i[1]] for i in result]
    dict_blocal_rank = []

    for i in local_rank:
        dict_blocal_rank.append({'name': i[1], 'ranking': i[0], 'point': i[2]})
...
def belong_ranking(belong, point, user_id):
    ...
    belong_rank = [[i[2], i[0], i[1]] for i in result]
    dict_belong_rank = []

    for i in belong_rank:
        dict_belong_rank.append({'name': i[1], 'ranking': i[0], 'point': i[2]})
...
def personal_ranking(point, user_id):
    ...
    personal_rank = [[i[2], i[0], i[1]] for i in result]
    dict_personal_rank = []

    for i in personal_rank:
        dict_personal_rank.append({'id': i[1], 'ranking': i[0], 'point': i[2]})
